...
public class JAnimatedIcon extends JLabel {
	 ...
	 * 
	 * @param gif
	 *            le chemin vers l'animation gif
	 * @param milli
	 *            attente en milliseconde entre 2 images
	 */
	...
	/**
	 * constructeur <br>
	 * Copie de l'icone en redimensionnant
	 * 
	 * @param a
	 *            le JAnimatedIcon a copier
	 * @param wi
	 *            la largeur en pixels voulue
	 * @param hi
	 *            la hauteur en pixels voulue
	 */
	public JAnimatedIcon(boolean bool, JAnimatedIcon a, int wi, int hi) {
		this(a, bool);
		reDimension(wi, hi);
		createAnim();
	}
	...
	/**
	 * constructeur <br>
	 * Copie de l'icone passé en parametre
	 * 
	 * @param a
	 *            le JAnimatedIcon a copier
	 */
	public JAnimatedIcon(JAnimatedIcon a, boolean bool) {
		a.copyIn(this);
		if (bool)
			createAnim2();
		else
	...
	/**
	 * constructeur <br>
	 * Copie de l'icone passé en parametre avec transparence
	 * 
	 * @param a
	 *            le JAnimatedIcon a copier
	 * @param key
	 ...
	 * @param tolerance
	 *            la tolerance sur la couleur transparente (comprise entre 0 et
	 *            1)
	 */
	...
	/**
	 * constructeur <br>
	 * Copie de l'icone en redimensionnant et avec une couleur transparente
	 * 
	 * @param a
	 *            le JAnimatedIcon a copier
	 * @param wi
	 ...
	 *            1)
	 */
	public JAnimatedIcon(JAnimatedIcon a, int wi, int hi, Color key,
			float tolerance) {
		// this(a);
		reDimension(wi, hi);
		createAnim();
	 ...
	 *            1)
	 */
	public JAnimatedIcon(String text, BufferedImage images[], Color key,
			float tolerance) {
		super(text);
	 ...
	 *            le texte
	 * @param images
	 *            les chemins des images de l'animation
	 */
	 ...
	 *            1)
	 */
	public JAnimatedIcon(String text, String images[], Color key,
			float tolerance) {
		super(text);
	 ...
	 *            le texte
	 * @param image
	 *            le chemin de l'image de l'animation
	 * @param key
	 *            la couleur transparente
	 * @param tolerance
	 *            la tolerance sur la couleur transparente (comprise entre 0 et
	 ...
	 *            1)
	 */
	public JAnimatedIcon(String text, BufferedImage image, Color key,
			float tolerance) {
		super(text);
	 ...
	 *            le texte
	 * @param image
	 *            chemin de l'image de l'animation
	 * @param iw
	 *            la largeur en pixels voulue
	 * @param ih
	 *            la hauteur en pixels voulue
	 ...
	 * 
	 * @param milli
	 *            duree d'attente en millisecondes entre 2 images de l'animation
	 */
	 ...
	 * 
	 * @param si
	 *            pause=true, met en pause sinon en sort
	 */
	...
	/**
	 * definie l'image de l'anim <br>
	 * Anim avec 1 seule image
	 * 
	 * @param image
	 *            l'image
	 */
	...
	/**
	 * decoupe les images de l'anim a partir d'une grande image (decoupe
	 * horizontale)
	 * 
	 * @param big
	 *            le chemin vers la "grosse" image contenant toutes les images
	 * @param iw
	 *            largeur des images de l'animation
	 * @param ih
	 *            hauteur des images de l'animation
	 */
	 ...
	 * 
	 * @param big
	 *            la "grosse" image contenant toutes les images
	 * @param iw
	 *            largeur des images de l'animation
	 * @param ih
	 *            hauteur des images de l'animation
	...
	private void nextFrame() {
		...
		if (img.length < 2 && c_img >= 0)
			return; // animation avec moins de 2 images

		if (duration > 0) {
			if (debut > 0) {
				if ((((int) System.currentTimeMillis()) - debut) >= duration) // duree
																				// depassée
				{
					setPause(true);
					c_img = -1;
	...
	/**
	 * appellé a la destruction de la classe <br>
	 * EXPERIMENTAL
	 */
	...
	/**
	 * sauvegarde d'une image en format approprié pour un animated icon
	 * (JAnimatedIcon) <br>
	 * les images de l'animation doivent etre alignées horizontalement et
	 * doivent faire la meme taille
	 * 
	 * @param wi
	 *            largeur d'une animation
	 * @param hi
	 *            hauteur d'une animation
	 * @param milli
	 *            attente en milliseconde entre chaque image
	 */
	public static boolean saveAnimatedIcon(BufferedImage img, String dest,
			int wi, int hi, int milli) {
		byte header[] = { 'A', 'n', 'i', 'm', 'a', 't', 'e', 'd', '*', 'I',
				'c', 'o', 'n' }; // l'entete
	...
	/**
	 * charge une image en format approprié pour un animated icon
	 * (JAnimatedIcon) (sauvegardé avec saveAnimatedIcon)
	 * 
	 * @param src
	 *            le fichier animated icon créé avec saveAnimatedIcon
	 * @param key
	 *            la couleur transparente
	 * @param tolerance
	 *            la tolerance sur la couleur transparente (comprise entre 0 et
	 ...
	 */
	public static JAnimatedIcon loadAnimatedIcon(String src, Color key,
			float tolerance) {
	...
	public void reDimension(int wi, int hi) {
		...
		for (int i = 0; i < img.length; i++) {
			...
			BufferedImage in = new BufferedImage(wi, hi,
					BufferedImage.TYPE_INT_ARGB); // io.getType());
	...
	public String toString() {
		return "JAnimatedIcon - number of frames : " + img.length
				+ " - frame rate : " + anim_every;
	}
	...
	public void setColorKeyTransparency(Color key, float tolerance) {
		...
		for (int i = 0; i < img.length; i++) {
			Image io = img[i].getImage();
			BufferedImage in = new BufferedImage(io.getWidth(null),
					io.getHeight(null), BufferedImage.TYPE_INT_ARGB);
			Graphics g = in.createGraphics();
	...
	/**
	 * definie une couleur transparente sur l'image les pixels de cette couleur
	 * ne seront pas affichés lors de drawImage UNIQUEMENT POUR UNE IMAGE DE
	 * TYPE TYPE_INT_ARGB !!! tolerance : 0.0f<=tol<=1.0f
	 * 
	 * @param img
	 *            l'image a traiter
	 * @param key
	 *            la couleur transparente
	 * @param tolerance
	 *            la tolerance (comprise entre 0 et 1)
	 */
	private static void setColorKey(BufferedImage img, Color key,
			float tolerance) {
		if (img == null)
			return;
		...
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				...
				if (r >= rkmin && r <= rkmax && g >= gkmin && g <= gkmax
						&& b >= bkmin && b <= bkmax) {
	 ...
	 * 
	 * @param format
	 *            le format désiré (voir ImageIO.write)
